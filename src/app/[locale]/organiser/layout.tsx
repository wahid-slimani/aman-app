...
export default async function OrganiserLayout({ children, params }: OrganiserLayoutProps) {
  ...
  const navItems = [
    { href: `/${locale}/organiser`, label: dict["nav.overview"], icon: <GridIcon />, exact: true },
    { href: `/${locale}/organiser/aid-points`, label: dict["nav.aidPoints"], icon: <PinIcon /> },
    { href: `/${locale}/organiser/analytics`, label: dict["nav.analytics"], icon: <ChartIcon /> },
  ];
  ...
  return (
    <div className="min-h-dvh bg-[#f7f9f7]" dir={localeDirection(locale)} id="organiser-shell" lang={locale}>
      <header className="sticky top-0 z-40 flex h-14 items-center justify-between border-b border-[#dfe7df] bg-white px-4 shadow-sm md:px-6">
        <Link className="flex items-center gap-2 font-semibold text-[#006233]" href={`/${locale}/organiser`}>
