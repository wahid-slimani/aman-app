...
export default async function AdminLayout({ children }: AdminLayoutProps) {
  ...
  const navItems = [
    { href: "/admin", label: dict["nav.overview"], icon: <GridIcon />, exact: true },
    { href: "/admin/aid-points", label: dict["nav.aidPoints"], icon: <PinIcon /> },
    { href: "/admin/organisers", label: dict["nav.organisers"], icon: <UsersIcon /> },
    { href: "/admin/reports", label: dict["nav.reports"], icon: <FlagIcon /> },
    { href: "/admin/dataset", label: dict["nav.dataset"], icon: <DatabaseIcon /> },
    { href: "/admin/analytics", label: dict["nav.analytics"], icon: <ChartIcon /> },
    { href: "/admin/audit", label: dict["nav.audit"], icon: <ShieldIcon /> },
  ];

  return (
    <div className="min-h-dvh bg-[#f7f9f7]" dir={localeDirection(locale)} id="admin-shell" lang={locale}>
      <header className="sticky top-0 z-40 flex h-14 items-center justify-between border-b border-[#dfe7df] bg-white px-4 shadow-sm md:px-6">
        <Link className="flex items-center gap-2 font-semibold text-[#006233]" href="/admin">
