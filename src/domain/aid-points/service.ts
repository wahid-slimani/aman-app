import { getAidPointByPublicSlug, findNearbyAidPoints, findPublishedAidPoints } from "@/infrastructure/repositories/aid-point-repository";
...
export async function listConfirmedAidPoints(input: {
  latitude: number;
  longitude: number;
}) {
  const points = await findPublishedAidPoints(input);
  const thresholds = await getVerificationThresholdSettings();
  return points.map((point) => ({
    ...point,
    verificationFreshness: classifyVerificationFreshness(point.lastVerifiedAt, thresholds)
